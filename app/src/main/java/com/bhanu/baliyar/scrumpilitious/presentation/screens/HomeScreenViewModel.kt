...
@HiltViewModel
class HomeScreenViewModel @Inject constructor(val getRecipesUseCase: GetRecipesUseCase) :
    ViewModel() {
        ...
        viewModelScope.launch {
            when (val result = getRecipesUseCase()) {
                is ResultWrapper.Success -> _state.value =
                    HomeScreenState.Success(result.response.recipes)

                is ResultWrapper.Error -> _state.value =
                    HomeScreenState.Error(
                        result.message ?: "There was an error processing the response"
                    )
            }
        }
    }
